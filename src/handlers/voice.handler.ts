...
function buildVoicePrompt(userRequest: string): string {
  return `Запрос: "${userRequest}"

AI-стартап для РФ. 2-5 чел, до 500к₽, MVP 1-3 мес.

ФОРМАТ:
🎯 ПРОБЛЕМА: [1-2 предложения] конкретная боль + цифры потерь
💡 РЕШЕНИЕ: [1-2 предложения] AI-технология с названием + как работает
💰 МОНЕТИЗАЦИЯ: [1 предложение] бизнес-модель + средний чек в ₽
📈 ROI: [1 предложение] экономия клиента в ₽ и часах

ТРЕБОВАНИЯ:
✅ Конкретная AI-технология с деталями
...
export async function voiceHandler(ctx: BotContext): Promise<void> {
  ...
  try {
    ...
    logger.info(
      `Checking Yandex credentials: apiKey=${config.yandexApiKey ? "SET" : "NOT SET"}, folderId=${config.yandexFolderId ? "SET" : "NOT SET"}`
    );

    if (!config.yandexApiKey || !config.yandexFolderId) {
      logger.error(
        `Missing Yandex credentials: apiKey=${!!config.yandexApiKey}, folderId=${!!config.yandexFolderId}`
      );
      await ctx.reply(
