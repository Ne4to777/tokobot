...
export class AIService {
  ...
  private buildPrompt(topic?: string): string {
    let prompt =
      ...
      "СТРУКТУРА ОТВЕТА (4-5 предложений, БЕЗ воды):\n" +
      "1. ПРОБЛЕМА: Какую конкретную боль решаем (с цифрами если возможно)\n" +
      "2. РЕШЕНИЕ: Как именно AI решает эту проблему (конкретная технология/подход)\n" +
      "3. МОНЕТИЗАЦИЯ: Бизнес-модель (подписка/комиссия/лицензия + примерный чек)\n" +
      "4. ROI: Какую выгоду получает клиент (экономия времени/денег в % или часах)\n\n" +
      "ТРЕБОВАНИЯ:\n" +
