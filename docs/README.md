# 📚 Документация Tokobot

Добро пожаловать в документацию проекта Tokobot!

## 🚀 Начало работы

Если вы впервые здесь, начните с этих документов:

### Для быстрого старта
- **[Quick Start Guide](QUICKSTART.md)** ⚡ - Запустите бота за 5 минут
- **[Навигация по проекту](NAVIGATION_GUIDE.md)** 🧭 - Найдите нужный файл быстро

### Для разработчиков
- **[Development Guide](DEVELOPMENT.md)** 💻 - Полное руководство разработчика
- **[Deployment Explained](DEPLOYMENT_EXPLAINED.md)** 🚀 - Как работает на сервере
- **[Project Structure](PROJECT_STRUCTURE.md)** 📁 - Структура репозитория
- **[Architecture](ARCHITECTURE.md)** 🏛️ - Архитектура системы

### Для контрибьюторов
- **[Contributing Guidelines](../CONTRIBUTING.md)** 🤝 - Как внести вклад
- **[Code of Conduct](../.github/CODE_OF_CONDUCT.md)** 📜 - Правила сообщества
- **[What Changed](WHAT_CHANGED.md)** 📝 - Последние изменения

## 📖 Полный список документов

### Getting Started
| Документ | Описание |
|----------|----------|
| [QUICKSTART.md](QUICKSTART.md) | 5-минутный гайд по запуску |
| [NAVIGATION_GUIDE.md](NAVIGATION_GUIDE.md) | Навигация по репозиторию |

### Development
| Документ | Описание |
|----------|----------|
| [DEVELOPMENT.md](DEVELOPMENT.md) | Полный гайд разработчика |
| [DEPLOYMENT_EXPLAINED.md](DEPLOYMENT_EXPLAINED.md) | Как работает на сервере (serverless) |
| [ARCHITECTURE.md](ARCHITECTURE.md) | Архитектура системы |
| [PROJECT_STRUCTURE.md](PROJECT_STRUCTURE.md) | Структура файлов проекта |

### Integration
| Документ | Описание |
|----------|----------|
| [BITRIX24_INTEGRATION.md](BITRIX24_INTEGRATION.md) | Интеграция с Bitrix24 CRM |

### Reference
| Документ | Описание |
|----------|----------|
| [REPOSITORY_SETUP.md](REPOSITORY_SETUP.md) | Настройка репозитория |
| [WHAT_CHANGED.md](WHAT_CHANGED.md) | Детальный отчет изменений v1.1.0 |

### Root Files (в корне проекта)
| Документ | Описание |
|----------|----------|
| [README.md](../README.md) | Главная страница проекта |
| [CONTRIBUTING.md](../CONTRIBUTING.md) | Правила контрибуции |
| [CHANGELOG.md](../CHANGELOG.md) | История версий |
| [LICENSE](../LICENSE) | MIT лицензия |

### GitHub Files (в .github/)
| Документ | Описание |
|----------|----------|
| [CODE_OF_CONDUCT.md](../.github/CODE_OF_CONDUCT.md) | Кодекс поведения |
| [SECURITY.md](../.github/SECURITY.md) | Политика безопасности |
| [AI_DEVELOPMENT_GUIDE.md](../.github/AI_DEVELOPMENT_GUIDE.md) | Гайд для AI-агентов |

## 🎯 Быстрые ссылки

### Я хочу...

**Быстро запустить проект:**  
→ [QUICKSTART.md](QUICKSTART.md)

**Понять как устроена система:**  
→ [ARCHITECTURE.md](ARCHITECTURE.md)

**Начать разработку:**  
→ [DEVELOPMENT.md](DEVELOPMENT.md)

**Внести изменения:**  
→ [CONTRIBUTING.md](../CONTRIBUTING.md)

**Настроить Bitrix24:**  
→ [BITRIX24_INTEGRATION.md](BITRIX24_INTEGRATION.md)

**Найти нужный файл:**  
→ [NAVIGATION_GUIDE.md](NAVIGATION_GUIDE.md)

**Работать с AI-агентом:**  
→ [AI_DEVELOPMENT_GUIDE.md](../.github/AI_DEVELOPMENT_GUIDE.md)

## 🤖 Для AI-агентов

Если вы AI-ассистент (GitHub Copilot, Cursor, ChatGPT и т.д.), начните с:
1. [AI_DEVELOPMENT_GUIDE.md](../.github/AI_DEVELOPMENT_GUIDE.md) - Comprehensive guide
2. [PROJECT_STRUCTURE.md](PROJECT_STRUCTURE.md) - Где что находится
3. [ARCHITECTURE.md](ARCHITECTURE.md) - Как устроена система

## 📞 Нужна помощь?

- 🐛 [Report a Bug](https://github.com/nybble777/tokobot/issues/new?template=bug_report.md)
- 💡 [Request a Feature](https://github.com/nybble777/tokobot/issues/new?template=feature_request.md)
- 💬 [Ask a Question](https://github.com/nybble777/tokobot/issues/new)

---

**Совет:** Добавьте эту страницу в закладки для быстрого доступа! 🔖

**Не нашли что искали?** Проверьте [NAVIGATION_GUIDE.md](NAVIGATION_GUIDE.md)

