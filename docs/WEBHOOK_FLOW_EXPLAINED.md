# 🔄 Как работает webhook.refactored.ts

Детальное объяснение flow выполнения модульного webhook.

---

## 📋 Структура файла

```typescript
// 1. Импорты
import { Telegraf } from "telegraf";
import { config } from "../config/index.js";
import { middleware } from "../middleware/index.js";
import { handlers } from "../handlers/index.js";

// 2. Инициализация
const bot = new Telegraf<BotContext>(config.token);

// 3. Регистрация middleware
bot.use(errorHandlerMiddleware);   // ← Глобальная обработка ошибок
bot.use(loggingMiddleware);         // ← Логирование каждого запроса
bot.use(rateLimitMiddleware);       // ← Защита от спама

// 4. Регистрация команд
bot.command("start", startHandler);
bot.command("help", helpHandler);
bot.command("idea", ideaHandler);
bot.command("contact", contactHandler);

// 5. Обработка текста (для contact flow)
bot.on("text", handleContactFlow);

// 6. Экспорт serverless функции
export default async (req, res) => { ... }

// 7. Режим разработки
if (config.environment === "development") { ... }
```

---

## 🔄 Flow выполнения

### Production (Vercel Webhook Mode)

```
Пользователь → Telegram → Vercel
                            ↓
            ┌───────────────────────────────┐
            │  export default async (req)   │ ← Entry point
            └───────────┬───────────────────┘
                        │
                        ▼
            ┌───────────────────────────────┐
            │   bot.handleUpdate(req.body)  │ ← Telegraf обрабатывает
            └───────────┬───────────────────┘
                        │
                        ▼
            ╔═══════════════════════════════╗
            ║      MIDDLEWARE CHAIN         ║
            ║   (выполняются по порядку!)   ║
            ╚═══════════╤═══════════════════╝
                        │
        ┌───────────────┼───────────────┐
        │               │               │
        ▼               ▼               ▼
┌──────────────┐ ┌──────────────┐ ┌──────────────┐
│   Error      │ │   Logging    │ │ Rate Limit   │
│  Handler     │ │  Middleware  │ │  Middleware  │
└──────┬───────┘ └──────┬───────┘ └──────┬───────┘
       │                │                │
       │  try/catch     │  Log request   │  Check limit
       │  для всех      │  + duration    │  Max 10/min
       │                │                │
       └────────────────┴────────────────┘
                        │
                        ▼
            ╔═══════════════════════════════╗
            ║     COMMAND ROUTER            ║
            ║   (Telegraf определяет)       ║
            ╚═══════════╤═══════════════════╝
                        │
        ┌───────────────┼───────────────┬───────────────┐
        │               │               │               │
        ▼               ▼               ▼               ▼
┌──────────────┐ ┌──────────────┐ ┌──────────────┐ ┌──────────────┐
│  /start      │ │  /help       │ │  /idea       │ │  /contact    │
│  Handler     │ │  Handler     │ │  Handler     │ │  Handler     │
└──────┬───────┘ └──────┬───────┘ └──────┬───────┘ └──────┬───────┘
       │                │                │                │
       ▼                ▼                ▼                ▼
┌──────────────┐ ┌──────────────┐ ┌──────────────┐ ┌──────────────┐
│ Показывает   │ │ Показывает   │ │   Вызывает   │ │  Запускает   │
│ приветствие  │ │   справку    │ │  aiService   │ │ contact flow │
└──────┬───────┘ └──────┬───────┘ └──────┬───────┘ └──────┬───────┘
       │                │                │                │
       └────────────────┴────────────────┴────────────────┘
                        │
                        ▼
            ┌───────────────────────────────┐
            │      ctx.reply(message)       │ ← Ответ пользователю
            └───────────┬───────────────────┘
                        │
                        ▼
            ┌───────────────────────────────┐
            │    res.status(200).json()     │ ← Ответ Telegram
            └───────────────────────────────┘
                        │
                        ▼
                   Функция завершается
                   (serverless "засыпает")
```

### Development (Local Polling Mode)

```
┌───────────────────────────────────────┐
│  npm run dev                          │
│  tsx watch api/webhook.refactored.ts  │
└───────────────┬───────────────────────┘
                │
                ▼
┌───────────────────────────────────────┐
│  config.environment === "development" │ ← Проверка
└───────────────┬───────────────────────┘
                │ true
                ▼
┌───────────────────────────────────────┐
│  bot.launch()                         │ ← Long polling
└───────────────┬───────────────────────┘
                │
                ▼
        ╔═══════════════════╗
        ║  Bot Process      ║
        ║  Работает         ║
        ║  постоянно        ║
        ╚═══════╤═══════════╝
                │
     ┌──────────┴──────────┐
     │                     │
     ▼                     ▼
Опрашивает            Обрабатывает
Telegram              сообщения
каждые N сек          через тот же
                      middleware chain
     │
     └─→ Ctrl+C → bot.stop() → Завершение
```

---

## 🧩 Компоненты в деталях

### 1. Entry Point (строки 56-77)

```typescript
export default async (req: any, res: any) => {
  // Это функция вызывается Vercel при каждом POST от Telegram

  if (req.method === "POST") {
    // Telegram webhook - обрабатываем сообщение
    await bot.handleUpdate(req.body, res);
    res.status(200).json({ ok: true });
  } else if (req.method === "GET") {
    // Health check - для проверки что бот работает
    res.status(200).json({
      status: "ok",
      message: "Tokobot is running! (Refactored)",
      version: "2.0.0",
    });
  }
};
```

**Что происходит:**

1. Vercel получает POST от Telegram
2. Вызывает эту функцию с `req` и `res`
3. `bot.handleUpdate()` передает сообщение в Telegraf
4. Telegraf прогоняет через middleware chain
5. Вызывает нужный handler
6. Возвращает 200 OK Telegram

### 2. Middleware Chain (строки 31-34)

```typescript
bot.use(errorHandlerMiddleware); // 1️⃣
bot.use(loggingMiddleware); // 2️⃣
bot.use(rateLimitMiddleware); // 3️⃣
```

**Порядок выполнения КРИТИЧЕН!**

#### 1️⃣ Error Handler (первый)

```typescript
// middleware/error-handler.ts
export const errorHandlerMiddleware = async (ctx, next) => {
  try {
    await next(); // ← Выполняет все что дальше
  } catch (error) {
    await handleError(ctx, error); // ← Ловит ВСЕ ошибки
  }
};
```

**Зачем первый:** Оборачивает ВСЕ остальные middleware и handlers в try-catch.

#### 2️⃣ Logging (второй)

```typescript
// middleware/logging.ts
export const loggingMiddleware = async (ctx, next) => {
  logger.info(`Incoming message from user ${userId}`);
  const startTime = Date.now();

  await next(); // ← Выполняет handler

  const duration = Date.now() - startTime;
  logger.debug(`Processed in ${duration}ms`);
};
```

**Зачем второй:** Логирует каждый запрос с timing.

#### 3️⃣ Rate Limit (третий)

```typescript
// middleware/rate-limit.ts
export const rateLimitMiddleware = async (ctx, next) => {
  const userLimit = rateLimitStore.get(userId);

  if (userLimit.count >= MAX_REQUESTS) {
    await ctx.reply("⏱️ Слишком много запросов");
    return; // ← НЕ вызывает next() = прерывает chain
  }

  userLimit.count++;
  await next(); // ← Продолжает если лимит не превышен
};
```

**Зачем третий:** Защита от спама перед выполнением handler.

**Визуально:**

```
Request → Error Handler (try) →
          Logging (start timer) →
          Rate Limit (check) →
          Command Handler →
          Rate Limit (done) →
          Logging (log duration) →
          Error Handler (catch if error)
```

### 3. Command Registration (строки 36-40)

```typescript
bot.command("start", startHandler);
```

**Что это делает:**

- Регистрирует handler для команды `/start`
- Когда пользователь пишет `/start` → Telegraf вызывает `startHandler`

**Handler structure:**

```typescript
// handlers/start.handler.ts
export const startHandler: CommandHandler = async (ctx) => {
  // ctx = контекст (user, message, reply и т.д.)
  await ctx.reply("Привет!");
};
```

### 4. Text Handler (строка 43)

```typescript
bot.on("text", handleContactFlow);
```

**Когда срабатывает:**

- На ЛЮБОЕ текстовое сообщение (не команду)
- Используется для multi-step flow (contact form)

**Пример flow:**

```
User: /contact
Bot: Как вас зовут?
      ↓
User: Иван          ← bot.on("text") ловит
Bot: Укажите телефон
      ↓
User: +123456       ← bot.on("text") ловит
Bot: Заявка создана
```

### 5. Unknown Message Handler (строки 46-51)

```typescript
bot.on("message", async (ctx) => {
  await ctx.reply("❓ Команда не распознана");
});
```

**Когда срабатывает:**

- Если сообщение не подходит под предыдущие handlers
- Fallback для всех других сообщений

### 6. Development Mode (строки 82-106)

```typescript
if (config.environment === "development") {
  bot.launch(); // ← Long polling
}
```

**Как работает:**

- Проверяет `NODE_ENV`
- Если `"development"` → запускает polling
- Если НЕ development → просто экспортирует функцию

---

## 🔀 Order of Execution

### Пример: Пользователь пишет `/idea sales`

```
1. Telegram отправляет POST на Vercel
      ↓
2. Vercel вызывает: export default async (req, res)
      ↓
3. bot.handleUpdate(req.body)
      ↓
4. errorHandlerMiddleware: try {
      ↓
5. loggingMiddleware: logger.info("Incoming message...")
      ↓
6. rateLimitMiddleware: Check limit (OK)
      ↓
7. Telegraf router: Видит "/idea" → вызывает ideaHandler
      ↓
8. ideaHandler:
   - Parse args: topic = "sales"
   - await aiService.generateIdea({ topic: "sales" })
      ↓
9. aiService:
   - Пытается вызвать HuggingFace AI
   - Если ошибка → fallback на local ideas
   - Возвращает idea
      ↓
10. ideaHandler: await ctx.reply(idea)
      ↓
11. rateLimitMiddleware: done
      ↓
12. loggingMiddleware: logger.debug("Processed in 1234ms")
      ↓
13. errorHandlerMiddleware: } // no errors
      ↓
14. res.status(200).json({ ok: true })
      ↓
15. Function terminates
```

---

## 💡 Ключевые концепции

### 1. Middleware Chain

Middleware выполняются **последовательно** в порядке регистрации.

```typescript
bot.use(middlewareA); // Выполнится первым
bot.use(middlewareB); // Выполнится вторым
bot.use(middlewareC); // Выполнится третьим
```

### 2. next() Function

`await next()` передает управление следующему middleware/handler.

```typescript
export const myMiddleware = async (ctx, next) => {
  // До handler
  console.log("Before");

  await next(); // ← Выполняет следующий middleware/handler

  // После handler
  console.log("After");
};
```

### 3. Command Routing

Telegraf автоматически распознает команды и вызывает нужный handler.

```typescript
bot.command("start", handler1); // /start → handler1
bot.command("help", handler2); // /help → handler2
bot.on("text", handler3); // любой текст → handler3
bot.on("message", handler4); // fallback для всего остального
```

### 4. Context (ctx)

Объект `ctx` содержит:

- `ctx.from` - информация о пользователе
- `ctx.message` - сообщение
- `ctx.reply()` - отправить ответ
- `ctx.sendChatAction()` - показать "typing..."

### 5. Error Propagation

Ошибки "всплывают" вверх по chain пока не поймает errorHandlerMiddleware.

```
Handler throws error
     ↓
Rate Limit: no catch → passes up
     ↓
Logging: no catch → passes up
     ↓
Error Handler: catch! → handleError()
```

---

## 🆚 Сравнение с старым кодом

### Старый webhook.ts (монолитный)

```typescript
// ВСЁ В ОДНОМ ФАЙЛЕ
bot.command("start", async (ctx) => {
  try {
    // Логика команды здесь
    await ctx.reply("...");
  } catch (error) {
    console.error(error);
    await ctx.reply("Ошибка");
  }
});

bot.command("help", async (ctx) => {
  try {
    // Логика команды здесь
    await ctx.reply("...");
  } catch (error) {
    console.error(error);
    await ctx.reply("Ошибка");
  }
});

// ... еще 5 команд с copy-paste error handling
```

**Проблемы:**

- ❌ Дублирование кода (error handling в каждой команде)
- ❌ Нет логирования
- ❌ Нет rate limiting
- ❌ Сложно тестировать
- ❌ Все в одном файле (195 строк)

### Новый webhook.refactored.ts (модульный)

```typescript
// Middleware делают всю работу
bot.use(errorHandlerMiddleware); // Error handling для ВСЕХ
bot.use(loggingMiddleware); // Logging для ВСЕХ
bot.use(rateLimitMiddleware); // Rate limit для ВСЕХ

// Handler только с логикой
bot.command("start", startHandler); // 1 строка!
bot.command("help", helpHandler); // 1 строка!
```

**Преимущества:**

- ✅ Нет дублирования
- ✅ Централизованный error handling
- ✅ Автоматическое логирование
- ✅ Rate limiting из коробки
- ✅ Легко тестировать
- ✅ 108 строк vs 195

---

## 🎯 Итого

**webhook.refactored.ts работает так:**

1. **Инициализация** (при первом запуске)

   - Загружает config
   - Создает bot instance
   - Регистрирует middleware
   - Регистрирует handlers

2. **На каждый запрос** (Vercel webhook)

   - Vercel вызывает export default функцию
   - bot.handleUpdate() прогоняет через middleware chain
   - Middleware обрабатывают (error, log, rate limit)
   - Router вызывает нужный handler
   - Handler выполняет бизнес-логику
   - Ответ отправляется пользователю
   - Функция завершается

3. **В development режиме** (локально)
   - Запускает polling
   - Постоянно опрашивает Telegram
   - Использует тот же middleware chain
   - Работает до Ctrl+C

**Ключевое отличие от старого:**

- Разделение ответственности
- Middleware для cross-cutting concerns
- Handlers только для бизнес-логики
- Services для внешних интеграций
- Все модульно и переиспользуемо

---

Последнее обновление: 2026-01-07
