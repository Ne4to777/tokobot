...
хостинг на Vercel с использованием Groq API (Llama 3.3 70B).
...
- 🎯 Идеи в конкретных областях: `/idea sales`, `/idea marketing`, `/idea hr` и
  др.
- 🤖 Интеграция с бесплатным AI (Groq Llama 3.3 70B) или локальная база из 50+ идей
...
### 2. (Опционально) Получите Groq API ключ
...
> - 🚀 Самая быстрая генерация (2-3 сек)
> - 🆓 Бесплатно 14,400 запросов/день
> - Бот работает и без API ключа, используя локальную генерацию идей
> - Также поддерживается Gemini (установите `AI_PROVIDER=gemini`, не работает в
>   РФ)
...
5. Добавьте переменные окружения:
   ...
   - `GROQ_API_KEY` - (опционально) API ключ от Groq
   - `AI_PROVIDER` - (опционально) `groq`, `gemini` или `huggingface`, по
     умолчанию `groq`
6. Нажмите "Deploy"
