...
хостинг на Vercel с использованием Google Gemini 2.0 Flash API.
...
- 🎯 Идеи в конкретных областях: `/idea sales`, `/idea marketing`, `/idea hr` и
  др.
- 🤖 Интеграция с бесплатным AI (Google Gemini 2.0 Flash) или локальная база из 50+ идей
...
### 2. (Опционально) Получите Google Gemini API ключ
...
5. Добавьте переменные окружения:
   ...
   - `GEMINI_API_KEY` - (опционально) API ключ от Google Gemini
   - `AI_PROVIDER` - (опционально) `gemini` или `huggingface`, по умолчанию `gemini`
6. Нажмите "Deploy"
...
- **Google Gemini 2.0** - Бесплатный AI API (рекомендуется)
...
1. Проверьте, что `GEMINI_API_KEY` установлен в Vercel
2. Проверьте лимиты API (1500 запросов/день для Gemini)
3. Убедитесь, что API ключ активен на https://ai.google.dev/
...
- [Google Gemini](https://ai.google.dev/) - Free AI API (recommended)
